const pixForm = document.getElementById('pixForm');
    ...
    pixForm.addEventListener('submit', async (event) => {
      ...
      const payload = {
        ...
        pixExpiresInDays: 2,
        postbackUrl: 'https://ofcqmengao.site/webhook-pagamento',
      ...
      try {
        const response = await fetch('https://ofcqmengao.site/gerar-pix', {
          method: 'POST',
    ...
    async function checkPaymentStatus() {
      ...
      try {
        const response = await fetch(`https://ofcqmengao.site/status-pagamento/${transactionId}`);
